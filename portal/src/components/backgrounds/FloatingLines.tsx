...
function frameRateIndependentLerpFactor(damping: number, deltaSeconds: number): number {
  const clampedDamping = Math.min(Math.max(damping, 0), 0.9999);
  if (clampedDamping <= 0) {
    return 0;
  }

  const rate = -60 * Math.log(1 - clampedDamping);
  return 1 - Math.exp(-rate * Math.max(deltaSeconds, 0));
}

function safeChannel(hex: string, start: number, end: number): number {
  const parsed = parseInt(hex.slice(start, end), 16);
...
function hexToVec3(hex: string): Vector3 {
  ...
  if (value.length === 3) {
    r = safeChannel(value[0] + value[0], 0, 2);
    g = safeChannel(value[1] + value[1], 0, 2);
    b = safeChannel(value[2] + value[2], 0, 2);
  } else if (value.length === 6) {
    r = safeChannel(value, 0, 2);
    g = safeChannel(value, 2, 4);
    b = safeChannel(value, 4, 6);
  }
...
export default function FloatingLines({
  ...
  const getLineCount = (waveType: WaveType): number => {
    if (typeof lineCount === 'number') {
      return Math.max(0, Math.min(MAX_LINES, lineCount));
    }
    ...
    return Math.max(0, Math.min(MAX_LINES, lineCount[index] ?? 6));
  };
  ...
  useEffect(() => {
    ...
    if (interactive) {
      const target = trackPointerOnWindow ? window : renderer.domElement;
    ...
    const renderLoop = () => {
      ...
      const smoothingFactor = frameRateIndependentLerpFactor(mouseDamping, deltaSeconds);

      uniforms.iTime.value = clock.getElapsedTime();

      if (interactive) {
        currentMouseRef.current.lerp(targetMouseRef.current, smoothingFactor);
        uniforms.iMouse.value.copy(currentMouseRef.current);

        currentInfluenceRef.current += (targetInfluenceRef.current - currentInfluenceRef.current) * smoothingFactor;
        uniforms.bendInfluence.value = currentInfluenceRef.current;
      }

      if (parallax) {
        currentParallaxRef.current.lerp(targetParallaxRef.current, smoothingFactor);
